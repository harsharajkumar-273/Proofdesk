...
const getPreviewCacheHeader = (ext: string): string =>
...
const versionLivePreviewAssets = (html: string, version?: string): string => {
  if (!version) {
    return html;
  }
...
export const createPreviewRouter = (): Router => {
  ...
  router.get('/:sessionId/*', async (req: Request<any>, res: Response): Promise<any> => {
    ...
    let previewRoot = activeSession?.previewPath
      ? path.resolve(activeSession.previewPath)
      : '';

    if (!previewRoot) {
      previewRoot = await ensurePreviewBundle({
        sessionId,
        outputPath,
        repoPath,
      });
