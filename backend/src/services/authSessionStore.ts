...
const toCookieHeader = (name: string, value: string, options: CookieOptions = {}): string => {
...
export const parseCookies = (cookieHeader: string = ''): Record<string, string> =>
  String(cookieHeader)
    ...
    .filter(Boolean)
    .reduce((cookies: Record<string, string>, entry) => {
...
const hasConfiguredSecret = (value: any): value is string =>
  typeof value === 'string' &&
  value.trim().length > 0 &&
  !PLACEHOLDER_VALUES.has(value.trim().toLowerCase());

const getSessionSecrets = (): string[] => {
...
const getPrimaryKey = (): Buffer | null => {
...
const decryptAccessToken = (record: any): string | null => {
...
const serializeSessionForStorage = (session: any) => {
...
const restoreSessionFromStorage = (entry: any) => {
...
class AuthSessionStore {
  ...
  attachSessionCookie(res: any, sessionId: string) {
  ...
  createOAuthState(res: any, cookieName: string = OAUTH_STATE_COOKIE_NAME) {
    const secure = process.env.PROOFDESK_SECURE_COOKIES === 'true';
  ...
  clearOAuthState(res: any, cookieName: string = OAUTH_STATE_COOKIE_NAME) {
    const secure = process.env.PROOFDESK_SECURE_COOKIES === 'true';
    res.append(
      'Set-Cookie',
