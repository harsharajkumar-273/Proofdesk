...
export const sendBuildCompleteNotification = async ({
  to,
  repoOwner,
  repoName,
  success,
  ...
  try {
    ...
    logger.info(`[Email] Build notification sent to ${to} for ${repoLabel}`, { to, repoLabel });
  } catch (err: any) {
    logger.warn(`[Email] Failed to send build notification to ${to}: ${err.message}`, { to, repoLabel, err });
  }
};
