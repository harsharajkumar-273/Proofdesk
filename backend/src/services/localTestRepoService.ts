...
const toBoolean = (value: any): boolean => String(value || '').toLowerCase() === 'true';
...
const readOpeningTagAttribute = (xml: string, tagName: string, attributeName: string): string | null => {
...
const parseXmlSections = (xml: string): ParsedXml => {
  ...
  const sections = [...xml.matchAll(/<section[^>]*title="([^"]+)"[^>]*>([\s\S]*?)<\/section>/gi)].map(
    (match) => {
      ...
      const body = match[2];
      const paragraphs = [...body.matchAll(/<paragraph>([\s\S]*?)<\/paragraph>/gi)].map((pMatch) =>
        escapeHtml(pMatch[1].trim())
      );
      const items = [...body.matchAll(/<item>([\s\S]*?)<\/item>/gi)].map((iMatch) =>
        escapeHtml(iMatch[1].trim())
      );
      return {
        title: sectionTitle,
        paragraphs,
...
const renderCourseHtml = ({ title, subtitle, sections }: ParsedXml, xmlSource: string): string => {
...
class LocalTestRepoService {
  ...
  isLocalTestToken(token: string): boolean {
  ...
  matchesRepo(owner: string, repo: string): boolean {
    return owner === this.getRepoOwner() && repo === this.getRepoName();
  }

  ensureRepo(owner: string, repo: string): void {
  ...
  async getTree(owner: string, repo: string, relativePath: string = ''): Promise<any[]> {
    this.ensureRepo(owner, repo);
  ...
  async updateFileContent(owner: string, repo: string, filePath: string, content: string, message: string = ''): Promise<any> {
    this.ensureRepo(owner, repo);
    const resolvedPath = this.resolveRepoFile(filePath);
  ...
  async copyRepositoryTo(destination: string): Promise<void> {
    await fs.rm(destination, { recursive: true, force: true });
    await fs.cp(this.getRepoPath(), destination, { recursive: true, force: true });
  }

  async buildOutput(repoPath: string, outputPath: string): Promise<any> {
