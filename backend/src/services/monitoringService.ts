...
const trimString = (value: any, maxLength: number = MAX_STRING_LENGTH): string => {
...
const getMonitoringEventsPath = (): string => getProofdeskDataPath('monitoring', 'events.jsonl');

const maybePostWebhook = async (payload: any, env: NodeJS.ProcessEnv = process.env): Promise<void> => {
...
export const isMonitoringEnabled = (env: NodeJS.ProcessEnv = process.env): boolean =>
  !FALSE_VALUES.has(String(env.PROOFDESK_MONITORING_ENABLED ?? 'true').trim().toLowerCase());

export const getMonitoringContextFromRequest = (req: any): any => ({
...
export const recordMonitoringEvent = async (event: MonitoringEvent = {}, env: NodeJS.ProcessEnv = process.env): Promise<any> => {
...
export const readRecentMonitoringEvents = async (options: { limit?: number } = {}, env: NodeJS.ProcessEnv = process.env): Promise<any[]> => {
  if (!isMonitoringEnabled(env)) {
    return [];
  }
