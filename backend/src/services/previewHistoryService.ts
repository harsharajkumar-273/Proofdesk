...
const getHistoryDir = (sessionId: string): string => getProofdeskDataPath('preview-history', sessionId);
const getHistoryManifestPath = (sessionId: string): string => path.join(getHistoryDir(sessionId), 'manifest.json');
const getSnapshotPath = (sessionId: string, snapshotId: string): string =>
  path.join(getHistoryDir(sessionId), `${snapshotId}.html`);
...
const summarizeTextChanges = (beforeText = '', afterText = ''): SnapshotChangeSummary => {
...
const readManifest = async (sessionId: string): Promise<PreviewSnapshotEntry[]> => {
  try {
...
const writeManifest = async (sessionId: string, snapshots: PreviewSnapshotEntry[]): Promise<void> => {
...
export const listPreviewSnapshots = async (sessionId: string): Promise<PreviewSnapshotEntry[]> => {
...
export const readPreviewSnapshotHtml = async (sessionId: string, snapshotId: string): Promise<string> =>
...
export const recordPreviewSnapshot = async ({
  ...
  entryFile: string;
  label?: string;
}): Promise<PreviewSnapshotEntry | null> => {
  ...
  let changeSummary: SnapshotChangeSummary = {
    changedLines: 0,
    addedLines: 0,
    removedLines: 0,
  ...
  const nextEntry: PreviewSnapshotEntry = {
    snapshotId,
    entryFile,
    label,
    createdAt,
