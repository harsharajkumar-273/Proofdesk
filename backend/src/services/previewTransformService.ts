export const PREVIEW_SHARED_ROOT_DIRS = [
  'images',
  'figure-images',
  'fonts',
  'mathbook-assets',
...
const rewriteBareRelativeAttributes = (content: string, sessionBase: string): string =>
  content.replace(
...
const rewriteCssUrls = (content: string, sessionBase: string, allowedRelativeDirs: string[] = []): string => {
  const shouldRewriteRelativeUrl = (url: string) =>
...
export const PROOFDESK_PRETEX_LAYOUT_FIX_VERSION = '2026-04-21-display-math-reserve-v5';

export const PROOFDESK_PRETEX_LAYOUT_FIX = String.raw`
...
.pretex-display{
  ...
  min-height:var(--proofdesk-pretex-display-height, 1.5em);
...
const removeExistingPreTeXtLayoutFix = (html: string): string =>
  html
    .replace(PROOFDESK_LAYOUT_STYLE_PATTERN, '')
    .replace(PROOFDESK_LAYOUT_SCRIPT_PATTERN, '');

export const injectLatestPreTeXtLayoutFix = (html: string): string => {
...
const buildPreviewHtml = (raw: string, sessionBase: string, { isKnowlFile }: { isKnowlFile: boolean }): string => {
...
const buildPreviewCss = (rawCss: string, sessionBase: string, { isSiteCssFile }: { isSiteCssFile: boolean }): string => {
...
export const transformPreviewFile = (relativePath: string, rawContent: string, sessionId: string): string => {
