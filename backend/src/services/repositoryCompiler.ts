...
function run(cmd: string): Promise<RunResult> {
  return new Promise((resolve, reject) => {
    exec(
      ...
      (err, stdout, stderr) => {
        if (err) reject({ stdout, stderr, err } as RunError);
...
async function ensureDockerImage(): Promise<boolean> {
  try {
...
export default class RepositoryCompiler {
   ...
   */
  async build({ owner, repo }: { owner: string; repo: string }): Promise<BuildResult> {
   ...
   * Serve built files
   */
  async getArtifact(sessionId: string, filePath: string): Promise<Buffer> {
    const session = this.sessions.get(sessionId);
    if (!session) throw new Error('Session not found');
