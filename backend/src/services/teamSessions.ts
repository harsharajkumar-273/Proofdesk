...
const redisKey = (code: string) => `proofdesk:team-session:${code}`;
...
export const normalizeTeamSessionCode = (value: any = ''): string =>
  String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '');

export const isValidTeamRepo = (repo: any): repo is TeamSessionRepo =>
  !!(repo &&
...
const generateTeamSessionCode = (): string => {
  let code = '';
...
class TeamSessionStore {
  ...
  async load(force: boolean = false): Promise<void> {
    ...
    try {
      ...
      const nextSessions = new Map<string, TeamSession>();
  ...
  async createSession({ repo, createdBy }: { repo: TeamSessionRepo; createdBy: any }): Promise<TeamSession> {
  ...
  async getSession(code: string): Promise<TeamSession | null> {
    await this.loadPromise;
    ...
    if (isRedisSharedStateEnabled()) {
      ...
      const session = JSON.parse(rawSession) as TeamSession;
