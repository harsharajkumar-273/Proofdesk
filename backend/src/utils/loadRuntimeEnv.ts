...
const findBackendRoot = (currentDir: string): string => {
  let dir = path.resolve(currentDir);

  for (let depth = 0; depth < 6; depth += 1) {
    if (
      fs.existsSync(path.join(dir, 'package.json'))
      && (fs.existsSync(path.join(dir, 'src', 'server.ts')) || fs.existsSync(path.join(dir, 'src', 'server.js')))
    ) {
      return dir;
    }
...
export const loadRuntimeEnv = (currentDir: string): void => {
