...
const hasValue = (value: any): boolean => typeof value === 'string' && value.trim().length > 0;
const isPlaceholderValue = (value: any): boolean => PLACEHOLDER_VALUES.has(String(value || '').trim().toLowerCase());
export const hasConfiguredValue = (value: any): boolean => hasValue(value) && !isPlaceholderValue(value);

const isAbsoluteHttpUrl = (value: any): boolean => /^https?:\/\//i.test(String(value || ''));

const isLocalHttpUrl = (value: any): boolean =>
  /^https?:\/\/(?:localhost|127(?:\.\d+){3})(?::\d+)?(?:\/|$)/i.test(String(value || ''));

const countCsvEntries = (value: any): number =>
  String(value || '')
    .split(',')
...
export interface RuntimeValidation {
  config: RuntimeConfig & { githubOauthConfigured: boolean };
  errors: Issue[];
  warnings: Issue[];
  ready: boolean;
}
...
export const formatRuntimeValidation = (validation: RuntimeValidation): string => {
  const lines: string[] = [];
...
export const getReadinessPayload = (env: NodeJS.ProcessEnv = process.env, options: { strict?: boolean } = {}): any => {
